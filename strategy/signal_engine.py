...
def _check_btc_market(btc_df: pd.DataFrame) -> str:
    ...
    # BTC 단기 하락 구조 + 강한 추세 — 알트 롱 위험
    btc_bear = (
        btc_trend.ema_short < btc_trend.ema_mid   # EMA20 < EMA50
        and btc_trend.adx_minus_di > btc_trend.adx_plus_di  # -DI 우세
        and btc_trend.adx >= 30                    # 추세가 실제로 있을 때만
    )
    if btc_bear:
        return (
